...
public class OtherEvaluation extends Evaluation {
	...
	public void crossValidateModel1(Classifier classifier, Instances data,
			int numFolds, Random random, Object... forPredictionsPrinting)
			throws Exception {
		...
		for (int i = 0; i < numFolds; i++) {
			...
			switch(choose){
				...
				case 3:{
					train=sample.AntiUnderSample(trainOrigin,0.2);
					break;
				}
				case 4:{
					train=sample.AntiOverSample(trainOrigin,0.2);
					break;
				}
				case 5:{
					train=sample.SmoteSample(trainOrigin, 1);
			...
			setPriors(train);
			Classifier copiedClassifier = Classifier.makeCopy(classifier);
			/*if(classifier.getClass().getName().contains("classify.base.NewIsolationForest")){
				((NewIsolationForest) copiedClassifier).setM_threshold(((NewIsolationForest) classifier).findThreshold((NewIsolationForest)classifier,train));
				System.out.println("threshold:" + ((NewIsolationForest)copiedClassifier).getM_threshold());
			}*/
			copiedClassifier.buildClassifier(train);
			Instances test = data.testCV(numFolds, i);
